...
class SyncOTAServer:
    ...
    def calculate_power_scalar(self, grad_norm_estimates):
        ...
        if min_grad_norm < 1e-8:
            min_grad_norm = 1e-8
            logger.warning("Using safe min grad norm")
        
        # Paper Eq 29: σ_t = γ₀σ₀²√s / minₙ(‖g̃ₙₜ‖₂)
        # But note dimension analysis requires:
        numerator = self.gamma0 * (self.sigma0**2) * self.s
        self.sigma_t = math.sqrt(numerator) / min_grad_norm
        
        logger.info(f"Power scalar: {self.sigma_t:.4f} | "
                    f"Min grad: {min_grad_norm:.4f} | "
                    f"Numerator: {numerator:.4e}")
        return self.sigma_t
    ...
    def aggregate_gradients(self, selected_clients):
        ...
        # Add Gaussian noise (correctly handles variance)
        noise = torch.randn(self.s, device=self.device) * math.sqrt(self.sigma0)
        ...
        logger.info(f"Aggregation complete | "
                    ...
                    f"Noise std: {math.sqrt(self.sigma0):.4f} | "
    ...
    def update_global_model(self, update, round_idx):
        ...
        if update is None:  # Explicit None check
    ...
    def update_virtual_queues(self, actual_energies):
        ...
        for client in self.clients:
            ...
            per_round_budget = float(self.E_bars[cid]) / self.T
    ...
    def run_round(self, round_idx):
        ...
        # Return metrics - FIXED THE TENSOR BOOLEAN ISSUE HERE
        round_metrics = {
